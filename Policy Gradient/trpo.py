...
@tf.function
def get_action(actor_model: tf.keras.Model, state: tf.Tensor) -> Tuple[tf.Tensor]:
...
def run_episode(
    ...
    for t in range(max_step):
        ...
        tf_actions = get_action(actor_model, tf_state)
...
def main():
    ...
    actor_model = Actor(action_shape, lower_bound, upper_bound)

    critic_model = get_critic_model(observation_shape)
    buffer = EpisodicBuffer()
