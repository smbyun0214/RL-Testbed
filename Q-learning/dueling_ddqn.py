...
def create_model(state_shape, n_actions):
    ...
    hidden_v = tf.keras.layers.Dense(units=32, activation=tf.keras.activations.relu)(inputs)
    hidden_adv = tf.keras.layers.Dense(units=32, activation=tf.keras.activations.relu)(inputs)
    outputs_v = tf.keras.layers.Dense(units=1)(hidden_v)
    outputs_adv = tf.keras.layers.Dense(units=n_actions)(hidden_adv)
    outputs_q = outputs_v + (outputs_adv - tf.math.reduce_mean(outputs_adv, axis=1, keepdims=True))
    return tf.keras.Model(inputs=inputs, outputs=outputs_q)
