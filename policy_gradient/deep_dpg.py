...
@tf.function
def get_action(state: tf.Tensor, actor_model: tf.keras.Model, noise: OUActionNoise, training: bool = True) -> Tuple[tf.Tensor]:
    if training:
        return actor_model(state) + noise()
    else:
        return actor_model(state)
...
def run_episode(
    ...
    for t in range(max_step):
        ...
        if buffer.get_length() > buffer.maxlen/5 and t % 10 == 0:
...
def main():
    ...
    actor_model = get_actor_model(observation_shape[0], action_shape[0], lower_bound, upper_bound)
    critic_model = get_critic_model(observation_shape[0], action_shape[0])
    
    target_actor_model = get_actor_model(observation_shape[0], action_shape[0], lower_bound, upper_bound)
    target_critic_model = get_critic_model(observation_shape[0], action_shape[0])
...
def save_video():
    ...
    model = get_actor_model(observation_shape, action_shape, lower_bound, upper_bound)
    model.load_weights(f"checkpoints/{algorithm}-{env_id}/latest")

    state = env.reset()
    while True:
        tf_state = tf.expand_dims(state, axis=0)
        action = get_action(tf_state, model, None, training=False)
